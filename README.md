...
| POST | `/v1/party-reference-data-directory-entry/initiate` | Initiate |
| GET | `/v1/party-reference-data-directory-entry` | Retrieve (list) |
| GET | `/v1/party-reference-data-directory-entry/{crId}/retrieve` | Retrieve |
| PUT | `/v1/party-reference-data-directory-entry/{crId}/update` | Update |
| PUT | `/v1/party-reference-data-directory-entry/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/party-reference-data-directory-entry/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-party-reference-data-directory:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-party-reference-data-directory ./helm -n bian-reference-data
```

Exposed through the platform Gateway at path prefix `/sd-party-reference-data-directory` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
